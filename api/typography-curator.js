...
function content(data){
 const c=[{type:"input_text",text:`Você é o CURADOR TIPOGRÁFICO. Você NÃO é o diretor visual. O Diretor de Arte decide posição, cor, alinhamento, hierarquia e contraste. Sua função principal é encontrar a fonte real mais próxima possível da intenção visual do Diretor/referência e recomendar fallback seguro. Nunca substitua uma decisão estética do typographyBlueprint por preferência própria. do ChurchDesign. Sua missão é decidir quando uma tipografia real/editável melhora a peça e quando destruiria a linguagem visual.
...
module.exports=async function handler(req,res){if(req.method!=="POST")return res.status(405).json({error:"Método não permitido."});try{const data=req.body||{};const r=await fetch(RESPONSES_URL,{method:"POST",headers:{Authorization:`Bearer ${process.env.OPENAI_API_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({model:"gpt-5.6-terra",reasoning:{effort:"low"},prompt_cache_options:{mode:"explicit",ttl:"30m"},store:false,input:[{role:"user",content:content(data)}],text:{format:{type:"json_schema",name:"churchdesign_typography_plan",strict:true,schema},verbosity:"low"}})});const requestId=r.headers.get("x-request-id")||null;const d=await r.json();if(!r.ok){const e=new Error(d?.error?.message||`OpenAI ${r.status}`);e.requestId=requestId;throw e;}let plan;try{plan=JSON.parse(out(d))}catch{throw new Error("Plano tipográfico inválido.");}
 ...
 return res.status(200).json({success:true,typographyPlan:plan,meta:{model:'gpt-5.6-terra',usage:d.usage||null,requestId,endpoint:'responses'}});}catch(e){console.error("ChurchDesign typography curator",e);return res.status(500).json({error:e.message||"Erro no Curador Tipográfico."});}};
